...
export type ContentType = "text" | "audio" | "video" | "image" | "email";
  ...
  return (
    <form onSubmit={onSubmit} className="w-full space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-center gap-1.5 p-1.5 bg-[var(--field)] backdrop-blur-md border border-[var(--line-ink)] rounded-xl shadow-inner">
        {(["text", "audio", "video", "image", "email"] as ContentType[]).map((type) => {
          ...
          const labels: Record<ContentType, { en: string; hi: string; icon: string }> = {
            ...
            email: { en: "Email / EML File",  hi: "ईमेल / EML फ़ाइल",   icon: "📧" },
          };
          ...
          return (
            <button
              ...
              className={`flex-1 min-w-[110px] py-2.5 px-3 text-xs font-mono tracking-wider uppercase transition-all rounded-lg flex items-center justify-center gap-2 ${
                isActive
      ...
      {contentType === "text" ? (
        <div className="space-y-2">
          <textarea
            ...
            className="w-full p-4 text-xs font-mono bg-[var(--field)] text-[var(--ink)] placeholder-[var(--ink-soft)] border border-[var(--line-ink)] rounded-xl shadow-inner focus:border-[var(--engrave)] focus:ring-2 focus:ring-[var(--engrave)]/30 outline-none resize-none transition-all disabled:opacity-60"
          />
        </div>
      ) : (
        <div className="relative flex flex-col items-center justify-center p-8 bg-[var(--field)] rounded-xl border-2 border-dashed border-[var(--line-ink)] hover:border-[var(--engrave)] transition-colors group cursor-pointer shadow-inner">
          ...
          <div className="text-3xl mb-2 group-hover:scale-110 transition-transform">
            {contentType === "audio" ? "🎧" : contentType === "video" ? "📼" : contentType === "email" ? "📨" : "🖼️"}
          </div>
          ...
          <span className="text-[10px] font-mono text-[var(--ink-soft)] mt-1 uppercase">
            {contentType === "email"
              ? "EML · TXT · MSG · JSON • Max 10MB"
              : `${contentType.toUpperCase()} • Max 50MB (Zero-Retention Enforced)`}
          </span>
        </div>
      )}

      {/* Dynamic Scan Progress HUD or Submit Button */}
      {isScanning ? (
        <ScanProgressBar
        ...
        <button
          type="submit"
          disabled={!textContent && !selectedFile}
